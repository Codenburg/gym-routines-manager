...
import { AdminDescuentoRowSkeleton } from "@/components/admin/skeletons/admin-descuento-row-skeleton";

/**
 * Per-page loading UI for `/admin/descuentos-duracion`.
 *
 * Mirrors the (admin) route group chrome (sidebar + main content
 * shell) and replaces the generic placeholder with the descuentos
 * page's actual layout: title + description + add form card +
 * descuentos list card with rows.
 *
 ...
 */
export default function AdminDescuentosLoading() {
  return (
      ...
      <div className="flex">
        ...
        <div className="ml-64 flex-1 p-6 hidden lg:block">
          <div className="container py-8 space-y-6">
            ...
            {/* Add descuento form card placeholder */}
            ...
            {/* Descuentos list card */}
            <div className="bg-secondary border border-border rounded-xl overflow-hidden">
              <div className="px-6 py-4 border-b border-border">
                <Skeleton className="h-5 w-56" />
              </div>
              {[1, 2, 3, 4].map((i) => (
                <AdminDescuentoRowSkeleton key={i} />
              ))}
            </div>
          </div>
        </div>
